...
Blockly.VariableMap.prototype.createVariable = function(name,
    opt_type, opt_init, opt_address, opt_id) {
  ...
  variable = new Blockly.VariableModel(this.workspace, name, opt_type, opt_init, opt_address, opt_id);
 ...
 * @returns {null}
 */
Blockly.VariableMap.prototype.editVariable = function (id, name, opt_value, opt_address) {
    // iterate over all types
    var keys = Object.keys(this.variableMap_);
    for (var i = 0; i < keys.length; i++ ) {
        var key = keys[i];
        // iterate over all variables of the specific type
        for (var j = 0, variable; variable = this.variableMap_[key][j]; j++) {
            if (variable.getId() == id) {
                this.variableMap_[key][j].address = opt_address;
                this.variableMap_[key][j].initValue = opt_value;
                this.variableMap_[key][j].name = name;

                Blockly.Events.fire(new Blockly.Events.VarRename(variable, name));
...
Blockly.VariableMap.prototype.changeVariableType = function(id, new_type) {
    var variable = this.getVariableById(id);
    if(variable != null && variable.type !== new_type){
        let index = this.variableMap_[variable.type].indexOf(variable);
        this.variableMap_[variable.type].splice(index, 1);
        variable.type = new_type;
        if(!this.variableMap_[new_type]){
          this.variableMap_[new_type] = [];
        }
        this.variableMap_[new_type].push(variable);
        Blockly.Events.fire(new Blockly.Events.VarRename(variable, variable.name));
        return;
    }
};
...
/**
 * Find the variable by the given name and return it. Returns null if
 *      it is not found. Searches through all types
 * @param {string} name The name to check for
 * @returns {Blockly.VariableModel} The variable with the given name, or null if it was not found.
 */
Blockly.VariableMap.prototype.getVariableByName = function(name){
    var keys = Object.keys(this.variableMap_);
    for (var i = 0; i < keys.length; i++ ) {
        var key = keys[i];
        // iterate over all variables of the specific type
