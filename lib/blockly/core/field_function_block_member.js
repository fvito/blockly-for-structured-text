 ...
 * @constructor
 */
Blockly.FieldFunctionBlockMember = function(varname, block, opt_type) {
...
Blockly.FieldFunctionBlockMember.dropdownCreate = function() {
    ...
    for (var i = 0; i < members.length; i++) {
        if(members[i].type === this.type_) {
            // Set the UUID as the internal representation of the variable.
            options.push([members[i].name, members[i].name]);
        }
    }
    return options;
};
